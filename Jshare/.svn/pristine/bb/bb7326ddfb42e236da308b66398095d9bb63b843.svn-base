...
public class SocialAction {
	...
	private final static Logger LOG = LogManager.getLogger(SocialAction.class);
	...
	public ResponseResultVo bind(@Lazy SSOUser ssoUser, @JsonModelAttribute @Validated({ BindGroup.class }) UserVo userBiding,
		...
		if (user == null) { // 如果用户不存在，则需要注册
			...
			com.digitalchina.web.common.share.vo.UserVo findUser = userBindManager.findBySocailId(socialId);
			if(ObjectHandler.isEmpty(findUser)){
				com.digitalchina.web.common.share.vo.UserVo userTemp = 
						com.digitalchina.web.common.share.vo.UserVo.createByUserId(user.getId().toString());
				userTemp.setName(user.getPhone().toString());
				userTemp.setNickname(user.getNickname());
				userTemp.setSocialId(socialId);
				userBindManager.doSave(userTemp);
			} else {
				findUser.setUserId(user.getId().toString());
				findUser.setSocialId(socialId);
				findUser.setName(user.getPhone().toString());
