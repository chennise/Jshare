...
public class ShareService extends AbstractCommonService{
	...
	public void applyShare(final ServiceContext serviceContext) {
		...
		try {
			...
			weixinRemote.sendTemplateMessage(recordVo.getPublisher().getSocialId(), recordVo, url, applicant.getNickname()+"申请了你的闲置，请查看\n", "分享中", "备注："+((applicant.getRemark()==null)?"无":applicant.getRemark())+"\n\n感谢您的使用"); // 发送模板消息
			...
			if(CollectionUtils.isNotEmpty(applicants)){
				for(UserVo applicant : applicants){
					if(StringUtils.equals(applicant.getId(), applicantId)){
						...
						weixinRemote.sendTemplateMessage(applicant.getSocialId(), recordVo, url, "恭喜！"+recordVo.getPublisher().getNickname()+"同意了你的申请，请查看\n", "申请成功", "联系方式："+ recordVo.getPublisher().getName()+"\n\n感谢您的使用"); // 发送模板消息
					}else{
						applicant.setApplyState(ShareConstants.APPLYSTATE_FAIL);
						weixinRemote.sendTemplateMessage(applicant.getSocialId(), recordVo, url, "很遗憾！你的申请没有通过\n", "申请失败", "\n感谢您的使用"); // 发送模板消息
					}
				}
			}
